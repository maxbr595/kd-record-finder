...
function openDiscogsShoppingPage(record: any) {
	window.open(`https://www.discogs.com/sell/release/${record.id}?ev=rb`, '_blank');
}
</script>
...
<template>
  <ScrollPanel class="w-full h-full">
    <div class="relative">
      ...
      <Timeline :value="discogsStore.groupedVariants(props.masterId)" layout="horizontal" class="w-max pt-3">
            ...
            <Card v-for="variant in item[1]" :key="variant.id" class="w-16rem">
              ...
              <template #content>
                <div class="text-xs">
                  ...
                  <p v-if="variant.label"><span class="font-bold">Label:</span> {{ variant.label.join(", ") }}</p>
                  <p><span class="font-bold">Land:</span> {{ variant.country || 'N/A' }}</p>
                </div>
              </template>
              <template #footer>
                <Button label="Discogs" link @click="openDiscogsShoppingPage(variant)" />
              </template>
            </Card>
          </div>
        </template>
